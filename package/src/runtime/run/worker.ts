...
import type { TaskExecutor } from "../task/index.js";
import { listPendingRuns, markRunDone, tryClaimRun } from "./queue.js";
...
export class RunWorker {
  ...
  private async execute(runId: string): Promise<void> {
    ...
    try {
      ...
      run.finishedAt = getTimestamp();
      run.output = { text: result.output };
      if (!result.success && result.error) run.error = { message: result.error };
      await saveRun(this.projectRoot, run);
    } catch (e) {
      run.status = "failed";
      run.finishedAt = getTimestamp();
