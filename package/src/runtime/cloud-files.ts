...
function safePublicRelativePath(input: string): string {
  ...
  // Accept both "uploads/a.pdf" and ".ship/public/uploads/a.pdf" for convenience.
  ...
  try {
    ...
    const tryOss = async () => {
      ...
      const bucket =
        ...
        String(params.storage?.bucket || "").trim();
    ...
    const tryPublic = async () => {
      ...
      const rel = safePublicRelativePath(
        String(params.publicPath || "").trim() || candidateKey,
      );
      ...
      const url = `${publicBaseUrl}${publicPrefix}/${encodePathForUrl(rel)}`.replace(/\/{2,}/g, "/").replace(":/", "://");
      return {
        ok: true as const,
        result: {
          success: true as const,
