 ...
 *
 * 同时定义“task run chatKey”格式，用于把 Agent 的 historyStore 映射到 run 目录：
 * - `task-run:<taskId>:<timestamp>`
 */

import path from "node:path";
import { getShipTasksDirPath } from "../../utils.js";
...
export function createTaskRunChatKey(taskId: string, timestamp: string): string {
...
export function parseTaskRunChatKey(chatKey: string): { taskId: string; timestamp: string } | null {
