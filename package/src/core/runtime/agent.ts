...
function extractToolCallsFromUiMessage(message: any): Array<{
  ...
  const parts = Array.isArray(message?.parts) ? message.parts : [];
  const out: Array<{ tool: string; input: Record<string, unknown>; output: string }> = [];
  ...
  for (const p of parts) {
    ...
    const rawInput =
      (p as any).input ?? (p as any).rawInput ?? (p as any).arguments ?? undefined;
  ...
  private bindChatKey(chatKey: string): string {
    ...
    try {
        ...
        systemExtras.push({
		      ...
		      try {
		        ...
		        if (pinnedSkillIds.length > 0) {
		          ...
		          const inputNormalized = Array.from(
		            new Set(pinnedSkillIds.map((x) => String(x || "").trim()).filter(Boolean)),
		          );
	      ...
	      const baseMaxInputTokensApprox =
	        typeof (this.configs.config as any)?.context?.history?.maxInputTokensApprox === "number"
		      ...
		      const maybePrunePinnedSkillsOnCompact = async (): Promise<void> => {
		        ...
		        try {
		          ...
		          const skillsList = Array.from(pinnedSkills.values()).map((s) => {
		            const preview = String(s.content || "").trim().slice(0, 400);
		            return {
		              id: s.id,
		              name: s.name,
		              path: s.skillMdPath,
		          ...
		          try {
		            ...
		            const arr = Array.isArray(parsed?.removeSkillIds) ? parsed.removeSkillIds : [];
		            removeIds = arr.map((x: any) => String(x || "").trim()).filter(Boolean);
		          } catch {
		            removeIds = [];
		          }
		      ...
		      let baseModelMessages: ModelMessage[] = (await historyStore.toModelMessages({ tools: this.tools })) as any;
		      ...
		      const reloadModelMessages = async (trigger: string): Promise<{ reloaded: boolean; drained?: number }> => {
		        if (typeof drainLaneMerged !== "function") return { reloaded: false };
		        try {
		          const r = await drainLaneMerged();
		          const drained = r && typeof (r as any).drained === "number" ? (r as any).drained : 0;
		          if (drained <= 0) return { reloaded: false, drained: 0 };
		          baseModelMessages = (await historyStore.toModelMessages({ tools: this.tools })) as any;
		          if (!Array.isArray(baseModelMessages) || baseModelMessages.length === 0) {
		            baseModelMessages = [{ role: "user", content: userText } as any];
		          }
		          needsHistoryResync = true;
		          void logger.log("debug", "Lane merged messages detected; reloaded history for next step", {
		            chatKey,
		            requestId,
		            trigger,
		            drained,
		          ...
		          if (onStep) {
	            try {
	              await emitStep("lane_merge", `drained=${drained}`, { requestId, chatKey, drained });
	            } catch {
	              // ignore
	            }
	          }
		      ...
		      const result = await withToolExecutionContext(
	            ...
	            return streamText({
		              ...
		              prepareStep: async ({ messages }) => {
		                ...
		                const incomingMessages: ModelMessage[] = Array.isArray(messages) ? (messages as any) : [];
		                const suffix =
                  ...
                  // 同时尽可能把 activeTools 缩到空集（防 provider 忽略 toolChoice 时仍能兜底）。
	                  activeTools = [];
		                ...
		                if (needsHistoryResync) {
		                  ...
		                  lastAppliedBasePrefixLen = Array.isArray(baseModelMessages) ? baseModelMessages.length : 0;
		                }
      ...
      for (const tc of toolCalls) {
        ...
        try {
          ...
          if (
            ...
            hadToolFailure = true;
            const err =
              (parsed as any).error || (parsed as any).stderr || "unknown error";
            toolFailureSummaries.push(`${tc.tool}: ${String(err)}`.slice(0, 200));
          }
        } catch {
          // ignore
        }
      ...
      return {
        ...
        toolCalls,
        ...(finalAssistantUiMessage ? { assistantMessage: finalAssistantUiMessage } : {}),
      };
    } catch (error) {
