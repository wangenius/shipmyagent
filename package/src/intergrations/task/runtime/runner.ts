...
async function runAgentRound(params: {
  ...
  const agent = getIntegrationContextManager(params.context).getAgent(params.contextId);
  const result = await getIntegrationRequestContextBridge(params.context).withContextRequestContext(
    {
      channel: "scheduler",
...
async function appendExecutorAssistantMessage(params: {
  ...
  const store = getIntegrationContextManager(params.context).getContextStore(params.runContextId);
  ...
  const userVisible =
    getIntegrationChatRuntimeBridge(params.context).pickLastSuccessfulChatSendText(
