...
id: core-boundary-review.md
source: local_dir
rel_path: core-boundary-review
kind: dir
description:  
is_expanded: true
---
...
- server/ / adapters/（入口层）依赖：chat/runtime/（调度 + 落盘 + 回包兜底）依赖：agent/context/（LLM tool-loop 执行引擎）
- chat/（调度 + store + egress）依赖：agent/（调用 Agent.run）
- agent/（prompt + tools + run）依赖：chat/history/store.ts（历史/compact）依赖：chat/context/request-context.ts（request scoped 元信息）依赖：chat/egress/（chat_send / dispatcher / idempotency）
...
- LLM 模型工厂（已做）
- egress helpers（已做）
- 下一步可以做：toolset 组装不再直接读取 ShipRuntimeContext（改为由 runtime/入口注入）history store / request context 放到更中立的 core 子域（避免 agent/chat 互相 import）
