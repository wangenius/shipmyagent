...
ShipMyAgent 的 MCP 配置不在 `ship.json` 里，而是在项目运行目录下的 `.ship/config/mcp.json`。
...
- `.ship/schema/mcp.schema.json`（并在 `mcp.json` 里写入 `$schema`，用于 IDE 校验/补全）
...
**context.contexts** (object, 可选)

用于控制每个 chatKey 的“工作上下文快照（contexts）”，落盘在 `.ship/chat/<chatKey>/contexts/`。

用途：
- 当背景过多/过乱时，agent 可以调用 `chat_context_new` 开一个新的工作上下文（旧上下文会落盘为快照）。
- 需要回忆某个旧上下文时，agent 可以调用 `chat_context_load` 通过文本检索并注入对应快照（仅供参考）。
