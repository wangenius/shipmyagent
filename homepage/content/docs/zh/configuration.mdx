...
ShipMyAgent 的 MCP 配置不在 `ship.json` 里，而是在项目运行目录下的 `.ship/mcp/mcp.json`。

详细说明见：[/docs/zh/mcp](/docs/zh/mcp)。
...
对象存储配置（S3 兼容，例如 Cloudflare R2）。

配置完成后，运行时会启用内置工具 `s3_upload`（Agent 可直接调用，用于把项目内文件上传到对象存储）。

| 字段 | 类型 | 必需 | 默认值 | 描述 |
|------|------|------|--------|------|
| `enabled` | boolean | 否 | true | 是否启用对象存储工具 |
| `provider` | string | 否 | `s3` | 目前仅支持 `s3` |
| `endpoint` | string | 是 | - | S3 Endpoint（R2 示例：`https://<account-id>.r2.cloudflarestorage.com`） |
| `accessKeyId` | string | 是 | - | Access Key ID（建议使用环境变量占位符） |
| `secretAccessKey` | string | 是 | - | Secret Access Key（建议使用环境变量占位符） |
| `region` | string | 否 | `auto` | SigV4 region（R2 通常是 `auto`） |
| `bucket` | string | 否 | - | 默认 bucket（用于 `cloud_file_upload` 等高层工具） |
...
#### cloudFiles (object, 可选)

云文件对外访问配置（用于生成 `/public/*` 的可访问 URL）。

| 字段 | 类型 | 必需 | 默认值 | 描述 |
|------|------|------|--------|------|
| `publicBaseUrl` | string | 否 | - | 你部署的 ShipMyAgent 服务的公网 Base URL（例如 `https://agent.example.com`） |
| `publicRoutePrefix` | string | 否 | `/public` | public 路由前缀 |
...
#### public 文件（.ship/public）

如果你把 SMA 部署到服务器上，还可以把文件放到项目的 `.ship/public/` 目录，并通过 HTTP 路由 `/public/*` 直接访问：

- 本地路径：`.ship/public/report.pdf`
- 访问路径：`https://<host>:<port>/public/report.pdf`

（该路由由服务器直接提供静态文件，不需要 OSS。）
...
```json
{
  "permissions": {
    "write_repo": {
      "paths": ["src/**", "tests/**"],  // Glob 模式
      "exclude": ["*.key", "secrets/**"],  // 排除模式
      "requiresApproval": true  // 写入前询问
    }
  }
}
```
...
```json
{
  "permissions": {
    "exec_shell": {
      "allow": ["ls *", "cat *", "find *", "rg *", "grep *", "git *", "npm *"],  // 命令模式
      "requiresApproval": false  // 自动允许安全命令
    }
  }
}
```
...
LLM 请求日志（通常包含 system + messages）可通过环境变量控制：

- `SMA_LOG_LLM_MESSAGES=0` 关闭
- `SMA_LOG_LLM_PAYLOAD=1` 在日志元信息中包含完整 payload（更敏感）
