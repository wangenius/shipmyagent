...
- **报错 `QQ requires chatType + messageId to send a reply`**：QQ 当前走的是“被动回复”接口，需要带上触发这次对话的 `messageId` 才能回消息。ShipMyAgent 会把最新收到的 `messageId` 记录在 `.ship/data/contact.json`，因此只有在对方先发过消息后才能通过 `chat_send` / `chat_contact_send` 回复。
