...
- **报错 `QQ requires chatType + messageId to send a reply`**：QQ 当前走的是“被动回复”接口，需要带上触发这次对话的 `messageId` 才能回消息。只要目标 chatKey 下已经有对方发过的消息（会落盘到 `.ship/chat/<encodedChatKey>/messages/history.jsonl`），工具就能从 history 里拿到最新 `messageId`；否则需要对方先发一条消息建立上下文。
