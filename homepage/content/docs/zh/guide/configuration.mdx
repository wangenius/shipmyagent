...
- **context.chatHistory**：从 `.ship/chat/<chatKey>/conversations/history.jsonl` 读取最近的 transcript，并以**一条 assistant message** 注入本次请求（更省 tokens、更稳定）。
  - **transcriptMaxMessages**: 注入的 user/assistant 最大条数（默认 `30`）
  - **transcriptMaxChars**: 注入内容最大字符数（默认 `12000`）
