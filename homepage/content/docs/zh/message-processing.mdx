...
用户消息会被 append 到 `.ship/chat/<chatKey>/conversations/history.jsonl`：
...
这个工具会从 `.ship/chat/<chatKey>/conversations/history.jsonl` 读取/搜索历史，并把 `user/assistant` 消息插入到当前上下文（插入位置在“当前 user 消息之前”，不改 user 原文）。
