...
历史上下文来自该 `chatKey` 的 history（`UIMessage[]`，落盘在 `.ship/chat/<encodedChatKey>/messages/messages.jsonl`），并在接近上下文窗口时自动 compact。
...
用户消息会被 append 到 `.ship/chat/<encodedChatKey>/messages/messages.jsonl`（JSONL，一行一条 `UIMessage`）。示例：
