...
| `shipmyagent start` | 启动 agent 服务器 |
| `shipmyagent alias` | 安装 shell alias `sma` |
| `shipmyagent stop` | 停止 agent 服务器 |
| `shipmyagent restart` | 重启 agent 服务器 |
...
也可以用更短的方式启动当前目录（等价于 `shipmyagent start .`）：
...
在 `~/.zshrc` 和/或 `~/.bashrc` 中写入 `alias sma="shipmyagent"`（幂等）。注意：CLI 进程无法直接刷新你当前的 shell 会话；请手动执行 `source ~/.zshrc` / `source ~/.bashrc`（或重新打开终端）。
