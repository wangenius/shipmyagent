...
`.ship/` 是 ShipMyAgent 在你的项目根目录下创建的**本地运行时工作目录**：用于落盘聊天历史、日志、幂等缓存、联系人簿、MCP 配置，以及持久化记忆（profile/chat memory）等。
...
### `.ship/data/contact.json`：联系人簿（工具发送用）

用途：将 `username/别名/昵称` 映射到可投递目标（如 `chatId`/`chatKey`/`messageId`），支持类似 `chat_contact_lookup`、`chat_contact_send` 这类工具在“脱离当前请求上下文”时仍能找到收件人。

使用方式：

1. 先通过 `toolset_load` 加载 `contact_book` ToolSet（一次性注册联系人簿相关 tools，并注入使用说明）
2. 再使用 `chat_contact_*` 工具：`chat_contact_lookup` / `chat_contact_list` / `chat_contact_set_nickname` / `chat_contact_upsert` / `chat_contact_remove` / `chat_contact_send`
...
- **谨慎删除**：`.ship/chat/`、`.ship/profile/`、`.ship/data/contact.json`（会丢失对话历史/持久化记忆/联系人映射）。
- **迁移项目**：如需迁移同一 agent 的本地状态到新机器，可按需拷贝 `.ship/chat/`、`.ship/profile/`、`.ship/data/contact.json`；不建议拷贝 `.ship/.cache/`（避免跨环境 offset/幂等状态混淆）。
