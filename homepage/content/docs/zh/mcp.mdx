---
title: MCP 配置
description: 配置 .ship/config/mcp.json 接入外部工具（Model Context Protocol）
---
...
MCP 工具会以 `server:toolName` 的形式暴露给 Agent。

> ⚠️ **简化模式（2026-02-03）**：当前发布的 `shipmyagent` 包已暂时移除审批流程；MCP 工具调用会直接执行。
