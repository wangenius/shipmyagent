...
如需控制 LLM 请求日志（通常包含 system + messages），可通过环境变量：

- `SMA_LOG_LLM_MESSAGES=0` 关闭请求日志
- `SMA_LOG_LLM_PAYLOAD=1` 在日志元信息里包含完整 payload（更敏感）
...
# 只看审批相关
jq -c 'select(.type == "approval") | {timestamp, message, details}' .ship/logs/$(date +%F).jsonl | tail -200
```
