...
2) **项目配置上下文（Config-level）**
   ...
   - 运行时内置默认提示词：`package/src/core/prompts/default-ship-prompts.txt`
...
- 读取项目 `Agent.md`（如果存在）
- 拼上内置默认提示词（行为约束 / 输出规则）
- 扫描并渲染 Skills 概览（默认 `.ship/skills`）
...
- `messages`：来自该 `chatKey` 的 history（`UIMessage[]`），落盘在 `.ship/chat/<encodedChatKey>/messages/messages.jsonl`
- 本轮 `user` 消息保持原文（不加前缀）
...
- Skills/MCP：把 SOP 和外部系统能力“工程化地接进来”（`.ship/skills` 与 `.ship/config/mcp.json`）
