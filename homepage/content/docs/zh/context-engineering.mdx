...
`createAgentRuntimeFromPath()`（`package/src/runtime/agent/factory.ts`）会构建 `agentMd`：

- 读取项目 `Agent.md`（不存在则用默认角色）
- 拼上内置默认提示词（`package/src/asset/prompts.txt`）
- 扫描并渲染 Skills 概览（默认 `.claude/skills`）

### 2) in-flight messages：DefaultPrompt + history + 用户原文

`AgentRuntime.run()`（`package/src/runtime/agent/runtime.ts`）每次请求都会：

- 生成 `DefaultPrompt`（`package/src/runtime/agent/prompt.ts`），包含 projectRoot/chatKey/requestId/来源等，并规定输出方式（如必须走 `chat_send`）
- 组装 messages：`system(Agent.md + DefaultPrompt) + history(in-memory) + user(原文)`
- user 消息保持原样（不加前缀）；需要更多上文时可调用 `chat_load_history` 注入历史消息

## 历史太长怎么办（简化版）

当前实现采用两条简单策略：

- in-memory history 只保留最近 N 条（固定上限裁剪）
...
- `exec_shell`（`package/src/runtime/tools/exec-shell.ts`）：在 `projectRoot` 下执行 `ls/cat/rg/git/test/build…`，按需读取/搜索仓库信息
- `ChatStore`（`package/src/runtime/chat/store.ts`）：将聊天记录 append 到 `.ship/chats/*.jsonl`；需要更多落盘历史时可用 `chat_load_history` 按需加载
- Skills/MCP：把 SOP 和外部系统能力“工程化地接进来”（`.claude/skills` 与 `.ship/mcp/mcp.json`）

---

如果你想看更“偏工程实现”的版本（带更多文件路径与职责拆分），也可以直接读仓库内的 `docs/agent-context-engineering.md`。
