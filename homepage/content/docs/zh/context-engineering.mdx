...
- 需要更多历史细节时，通过 `chat_load_history` 从 `.ship/chat/<chatKey>/conversations/history.jsonl` 按需读取/搜索并注入当前上下文
...
- `ChatStore`（`package/src/core/chat/store.ts`）：将聊天记录 append 到 `.ship/chat/<chatKey>/conversations/history.jsonl`；需要更多落盘历史时可用 `chat_load_history` 按需加载
