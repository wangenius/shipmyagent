...
Extend ShipMyAgent by integrating with external services like GitHub, Slack, Discord, and custom webhooks.
...
When the agent needs approval for sensitive operations:

```
🔔 Approval Required

Operation: Write to src/config.js
...
```json
{
  "integrations": {
    "telegram": {
      ...
      "botToken": "${TELEGRAM_BOT_TOKEN}",
      "chatId": "${TELEGRAM_CHAT_ID}",
      "webhookUrl": "https://your-server.com/webhook/telegram",
      "updates": {
        "timeout": 30,
        "limit": 10,
...
:::warning
Always configure `chatId` to prevent unauthorized access!
:::
...
Engage with your community on Discord.
