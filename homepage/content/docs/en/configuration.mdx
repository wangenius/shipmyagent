...
├── ship.json          # Main configuration (LLM, services)
...
**services.chat.queue** (object, optional)
...
- If new messages arrive while a run is in progress, they will be merged at step boundaries into the next step
...
**services.chat.egress** (object, optional)
...
Use `services.chat.adapters` to configure messaging platform adapters.
