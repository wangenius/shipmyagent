...
When configured, the runtime enables the built-in `s3_upload` tool (so the Agent can upload project-local files to your object storage).

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `enabled` | boolean | No | true | Enable object storage tools |
| `provider` | string | No | `s3` | Currently only `s3` is supported |
| `endpoint` | string | Yes | - | S3 endpoint (R2 example: `https://<account-id>.r2.cloudflarestorage.com`) |
| `accessKeyId` | string | Yes | - | Access key id (use env placeholders) |
| `secretAccessKey` | string | Yes | - | Secret access key (use env placeholders) |
| `region` | string | No | `auto` | SigV4 region (R2 typically uses `auto`) |
| `bucket` | string | No | - | Default bucket (used by higher-level tools like `cloud_file_upload`) |
...
#### cloudFiles (object, optional)

Cloud file public URL configuration (used to build accessible `/public/*` URLs).

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `publicBaseUrl` | string | No | - | Public base URL of your deployed ShipMyAgent server (e.g. `https://agent.example.com`) |
| `publicRoutePrefix` | string | No | `/public` | Public route prefix |

```json
{
  "cloudFiles": {
    "publicBaseUrl": "${SMA_PUBLIC_BASE_URL}",
    "publicRoutePrefix": "/public"
  }
}
```

#### Public files (`.ship/public`)

If you deploy SMA on a server, you can also put files under `.ship/public/` and access them via the HTTP route `/public/*`:

- Local path: `.ship/public/report.pdf`
- URL: `https://<host>:<port>/public/report.pdf`

(This is served directly by the server and does not require OSS.)
