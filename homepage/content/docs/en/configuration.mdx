...
- Transcript source: `.ship/chat/<chatKey>/conversations/history.jsonl`
...
**context.contexts** (object, optional)

Controls the optional per-chat **working context snapshots** stored under `.ship/chat/<chatKey>/contexts/`.

Use cases:
- When the conversation background becomes too long/noisy, the agent can start a **fresh working context** via `chat_context_new` (the old context is snapshotted to disk).
- Later, the agent can restore a snapshot by searching and injecting it via `chat_context_load` (reference-only).
