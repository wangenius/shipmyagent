...
For LLM request logging (may include system + messages), you can toggle in `ship.json`:

- `llm.logMessages=false` to disable request logging
...
tail -50 .ship/chat/<encode(chatKey)>/conversations/messages.jsonl
...
```bash
Note: there is no separate “payload logging” toggle in the current version. Logging is controlled solely by `llm.logMessages`.
```
