...
Different integrations/features generate different subdirectories; here's a typical structure (some directories/files may not exist):
...
Single line example from `history.jsonl` (fields vary slightly by platform/scenario):
...
- **Trigger conditions**:
  ...
  - Estimated tokens exceed `maxInputTokensApprox` (default 12000)
...
│       ├── history.jsonl       # History for this run
│       └── meta.json
```
