...
│                                                               │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │  Chat        │  │  Context     │  │  Long-term   │     │
│  │  Runtime     │  │  Compressor  │  │  Memory      │     │
...
#### 1. Chat Runtime Cache (by `chatKey`)

**Features**:
- Maintains independent in-memory `AgentRuntime` per `chatKey`
- Fast access to recent conversation content
- Automatic TTL cleanup of inactive chat runtimes to prevent memory leaks

**Working Mechanism**:
- Each `chatKey` stores data independently without interference
- Uses TTL cleanup for inactive `chatKey` runtimes (best-effort)
- Persists messages to disk via `ChatStore` (`.ship/chats/*.jsonl`)
...
```
.ship/
├── approvals/            # Pending approvals (human-in-the-loop)
│   ├── {approvalId}.json
│   └── ...
├── chats/                # Chat history (JSONL, append-only)
│   ├── {encode(chatKey)}.jsonl
│   └── ...
├── memory/              # Long-term memory storage
│   ├── {chatKey}.json
│   └── ...
├── .cache/              # Temporary cache (attachments, adapter state, etc.)
└── logs/                # Log files
    └── {date}.jsonl
```
...
4. **Chat Isolation**: Each `chatKey` managed independently without interference
