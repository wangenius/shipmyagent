...
1. **Configure OSS (S3-compatible)**: enables the built-in `s3_upload` tool to upload project-local files to object storage (Cloudflare R2 is S3-compatible).
...
Use env placeholders so you don’t commit secrets:
