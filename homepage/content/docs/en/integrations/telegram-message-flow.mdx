...
Before execution, the runtime uses an in-memory session history for the same `chatKey`. If the model needs more details from disk history, it can call `chat_load_history` to load and inject earlier messages from `.ship/chats/<chatKey>.jsonl`.
...
If the model decides it needs more context than the in-memory session provides, it can call `chat_load_history` to fetch earlier messages from `.ship/chats/<chatKey>.jsonl` and inject them into the current in-flight context (before the current user message).
