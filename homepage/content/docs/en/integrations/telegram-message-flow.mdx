...
Every incoming message is appended to a per-session JSONL file:

- `.ship/chats/<encode(sessionId)>.jsonl`
...
1. PermissionEngine writes `.ship/approvals/<id>.json` (including `meta` such as `source=telegram`, `userId`, `sessionId`, `initiatorId`)
